...
export class TokensService {
  ...
  async verifyAccessToken(token: string): Promise<{ sub: string }> {
    ...
    if (!accessToken) throw new TokenExpiredException()
  ...
  async verifyRefreshToken(token: string): Promise<{ sub: string }> {
    ...
    if (!tokens) throw new TokenExpiredException()
  ...
  disableTokens(access_token: string, all: boolean): void {
    console.log(all)
    if (all) return this.repository.disableRefreshToken(access_token)
